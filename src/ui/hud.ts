...
/** 一次画全 HUD：资源条 + 天数徽章 + 建造快捷栏。session 每帧调用这个即可。 */
...
export function drawDayBadge(ctx: CanvasRenderingContext2D, info: HudInfo): void {
  ...
  if (info.islanders) {
    ...
    const starving = info.islanders.fed < info.islanders.total;
    ctx.fillStyle = starving ? HUD_COLORS.danger : HUD_COLORS.accent;
    ctx.beginPath();
...
export function drawBuildBar(ctx: CanvasRenderingContext2D, info: HudInfo): void {
  ...
  // 栏上方提示行
  const hint = info.build?.hint ?? "按 1–5 选建筑 · 点海面放置";
  ctx.fillStyle = withAlpha(HUD_COLORS.ink, 0.55);
